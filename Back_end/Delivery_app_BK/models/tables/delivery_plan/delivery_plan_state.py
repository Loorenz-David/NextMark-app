...
from Delivery_app_BK.services.domain.plan.plan_states import PlanState as PlanStateEnum
...
class DeliveryPlanState (db.Model, TeamScopedMixin):
    ...
    def validate_name(self, key, value):
        if not value:
            raise ValidationFailed("DeliveryState name cannot be empty.")
        if value not in PlanStateEnum._value2member_map_:
            raise ValidationFailed(
                f"Invalid DeliveryState name '{value}'. "
