"""Local Delivery App - Order Plan Change Handler.

Handles route stop updates when an order is reassigned between local delivery plans.
"""
...
def apply_order_plan_change(
    ...
    Args:
        ctx: Service context
        order_instance: Order being reassigned
        old_plan: Previous delivery plan (or None if unassigned)
        new_plan: New delivery plan (or None if unassigning)
        apply_context: Pre-loaded context with local delivery plans and route solutions
    
    Returns:
        PlanChangeResult with updated stops and side effects
    """
...
def _merge_changed_route_solutions(
    ...
    """Merge and deduplicate route solutions."""
