"""Local Delivery App - Order Delete Extension Handler.

Handles route stop cleanup when an order is deleted from a local delivery plan.
"""
...
def apply_order_delete_extension(
    ...
    """Apply route stop cleanup when orders are deleted.
    
    Args:
        ctx: Service context
        delete_deltas: List of orders being deleted
        extension_context: Pre-loaded context with local delivery plans and route solutions
    
    Returns:
        OrderDeleteExtensionResult with removed stops and side effects
    """
