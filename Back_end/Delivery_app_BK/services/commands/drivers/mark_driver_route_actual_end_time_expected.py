...
from ._route_end_source import ROUTE_END_SOURCE_EXPECTED
...
def mark_driver_route_actual_end_time_expected(
    ctx: ServiceContext,
    route_id: int,
    ...
    if not can_record_route_timestamp(route_solution, candidate_time):
        reject_outside_window(ctx, "Expected route end was ignored because it is outside the route window.")
    ...
    recorded, reason = mark_route_end_time(
        ...
        source=ROUTE_END_SOURCE_EXPECTED,
    )
    if not recorded:
