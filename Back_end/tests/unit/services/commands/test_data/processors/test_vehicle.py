...
from Delivery_app_BK.services.commands.test_data.processors import vehicle as module
...
def test_process_calls_service_and_returns_id(monkeypatch):
    ...
    def fake_parse(payload):
        ...
        return SimpleNamespace(
            to_fields_dict=lambda: {"registration_number": "ABC-001", "team_id": 5}
        )

    def fake_create_instance(ctx, model, fields):
        return SimpleNamespace(id=43)

    monkeypatch.setattr(module, "parse_create_vehicle_request", fake_parse)
    ...
    assert result == 43
    assert calls == [{"registration_number": "ABC-001"}]
