...
export const ContactInfoStep = () => {
  ...
  const primaryPhone: Phone = data.client_primary_phone ?? { prefix: '+1', number: '' }
  const secondaryPhone: Phone = data.client_secondary_phone ?? { prefix: '+1', number: '' }

  const handleNext = () => {
    const newErrors: typeof errors = {}
    if (!data.client_email.trim()) newErrors.email = 'Email is required'
    if (!primaryPhone.number.trim()) newErrors.phone = 'Primary phone number is required'
    setErrors(newErrors)
  ...
  return (
    <div className="space-y-5">
      <div className="space-y-1">
        <h2 className="text-base font-semibold text-white/90">Contact Information</h2>
        <p className="text-sm text-white/46">How can the delivery team reach you?</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <label className="flex w-full flex-col gap-1.5">
          ...
          <div className={`custom-field-container${errors.email ? ' is-invalid' : ''}`}>
            <input
              type="email"
        ...
        <label className="flex w-full flex-col gap-1.5">
          ...
          <div className={`custom-field-container${errors.phone ? ' is-invalid' : ''}`}>
            <PhoneField
