...
export function useStopDetailController(stopClientId?: string) {
  ...
  const activeCasesCount = useMemo(() => {
    ...
    return typeof stop.order.open_order_cases === 'number' ? stop.order.open_order_cases : 0
  ...
  const sendAction = useCallback(async (
    type: 'arrive-stop' | 'complete-stop' | 'skip-stop' | 'fail-stop' | 'undo-stop-terminal',
    note?: string,
  ) => {
    ...
    showMessage({ status: 500, message: result.message ?? 'Unable to send route action.' })
    return result
  ...
  }, [nextStopClientId, openBottomSheet, openRouteStopDetail, snapBottomSheetTo])

  const runGuardedStopAction = useCallback(async (
    ...
    openOverlay('route-date-adjust-warning', {
      ...
      confirmLabel: 'Adjust dates to today',
      onConfirm: async () => {
        try {
  ...
  const navigateToStop = useCallback(async () => {
    ...
    const result = handleNavigateStopFlow({
      route,
      stop,
      arrivalRangeMeters: bootstrapState.routeTiming?.arrivalRangeMeters ?? null,
      services: {
        browserLocationService,
        mapAppPreferenceService,
        mapNavigationService,
    ...
    if (result.status === 'missing-destination') {
      showMessage({ status: 'warning', message: 'This stop has no usable destination for navigation.' })
    }
  }, [
  ...
  const callOrderPhone = useCallback(() => {
    ...
    if (result.status === 'missing-phone') {
      showMessage({ status: 'warning', message: 'This stop has no usable phone number.' })
    }
