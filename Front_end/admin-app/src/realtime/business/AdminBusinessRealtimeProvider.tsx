  ...
  useEffect(() => {
    ...
    const refreshPlanById = async (planId: number) => {
      ...
      const normalized = normalizeEntityMap<DeliveryPlan>(
        routePlanPayload as EntityMap<DeliveryPlan> | DeliveryPlan,
      );
      if (!normalized) {
        return;
      }
    ...
    const handleOrderChatEvent = (
      ...
      void runDedupedOrderRefresh(orderId, () => refreshOrderById(orderId));
      void runDedupedOrderCaseRefresh(orderId, () =>
        refreshOrderCasesByOrderId(orderId),
      );
    ...
    const handleOrderCaseEvent = (
      ...
      if (event.event_name === "order_case.state_changed" && orderCaseId) {
        const existingOrderCase = selectOrderCaseById(orderCaseId)(
          useOrderCaseStore.getState(),
        );
        const nextState = payload.state;
        if (existingOrderCase && typeof nextState === "string") {
          updateCaseState(
            orderCaseId,
            nextState as typeof existingOrderCase.state,
          );
        }
      }
      ...
      if (event.event_name === "order_case.created" && orderCaseId) {
        const clientId = String(
          payload.order_case_client_id ?? `order-case-${orderCaseId}`,
        );
        const existingOrderCase =
          selectOrderCaseById(orderCaseId)(useOrderCaseStore.getState()) ??
    ...
    const handlePlanEvent = (event: BusinessEventEnvelope<BusinessPayload>) => {
      ...
      const payload = event.payload ?? {};
      const planId =
        getPayloadNumber(payload, "delivery_plan_id") ?? event.entity_id;
      if (!planId) return;
    ...
    const handleClientFormSubmitted = (payload: ClientFormSubmittedPayload) => {
      const order = selectOrderByServerId(payload.order_id)(
      ...
      void runDedupedOrderRefresh(payload.order_id, () =>
        refreshOrderById(payload.order_id),
      );
    };

    const releaseAdminBusiness = adminBusinessChannel.subscribeTeamAdmin(
      (event) => {
