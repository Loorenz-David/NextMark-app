import { FloatingPopover } from '@/shared/popups/FloatingPopover/FloatingPopover'
import { InputField } from '@/shared/inputs/InputField'
import { BoldArrowIcon } from '@/assets/icons'

import { useVehicleSelectorContext } from './VehicleSelector.context'
import { VehicleSelectorList } from './VehicleSelectorList'

export const VehicleSelectorLayout = () => {
  const {
    isOpen,
    inputValue,
    selectedLabel,
    handleOpenChange,
    handleInputFocus,
    handleInputChange,
  } = useVehicleSelectorContext()

  const displayValue = selectedLabel ?? inputValue

  return (
    <FloatingPopover
      open={isOpen}
      onOpenChange={handleOpenChange}
      classes="relative w-full"
      matchReferenceWidth
      floatingClassName="z-[180]"
      reference={
        <div className="custom-field-container rounded-xl" onClick={handleInputFocus}>
          <InputField
            value={displayValue}
            onChange={handleInputChange}
            onFocus={handleInputFocus}
            placeholder="Select a vehicle"
            fieldClassName="w-full"
          />
          <div
            className="flex items-center content-center pr-2"
            onClick={() => {
              requestAnimationFrame(() => {
                handleOpenChange(!isOpen)
              })
            }}
          >
            <BoldArrowIcon className={`h-3 w-3 ${isOpen ? 'rotate-270' : 'rotate-90'}`} />
          </div>
        </div>
      }
    >
      <div className="admin-glass-popover rounded-2xl p-2 shadow-xl">
        <VehicleSelectorList />
      </div>
    </FloatingPopover>
  )
}
