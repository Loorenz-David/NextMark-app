...
import type { RouteGroupEditFormState } from '../forms/routeGroupEditForm/RouteGroupEditForm.types'
import type { RouteGroupEditFormWarnings } from '../forms/routeGroupEditForm/RouteGroupEditForm.types'
  ...
  const checkAvailability = useCallback(async () => {
    ...
    try {
      ...
      if (conflicts.length > 0) {
        ...
        const label = first.route_plan_label ?? `Plan ${first.route_plan_id}`
