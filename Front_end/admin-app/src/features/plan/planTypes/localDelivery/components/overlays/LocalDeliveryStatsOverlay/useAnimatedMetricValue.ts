...
import type { LocalDeliveryAnimatedMetric } from './LocalDeliveryStatsOverlay.types'

type UseAnimatedMetricValueOptions = {
  metric: LocalDeliveryAnimatedMetric | null | undefined
...
type UseAnimatedMetricValueResult = {
  value: number | null
  changeTick: number
  sourceType: LocalDeliveryAnimatedMetric['sourceType'] | null
  didChange: boolean
}

const easeOutCubic = (value: number) => 1 - Math.pow(1 - value, 3)

const hasMeaningfulChange = (previous: number, next: number, metric: LocalDeliveryAnimatedMetric) => {
...
export const formatAnimatedMetricValue = (
  metric: LocalDeliveryAnimatedMetric,
  numericValue: number,
): string => {
